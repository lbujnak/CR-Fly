...
class HTTPHelper {
    ...
    //tol - time out limit
    func httpPattern(url: String, tol: Double, sessionID : String?, completionHandler: @escaping (Data?,[String: Any]?, HTTPURLResponse?, Error?, Bool) -> Void){
        ...
        URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            else { completionHandler(data,jsonData,httpResponse,error,httpResponse!.statusCode / 100 != 2) }
        }.resume()
    }
