...
struct MeshWorkspaceView: View {
    
    @ObservedObject var appData: ApplicationData
    
    @State private var infoBar = false
    @State private var newProjectName = ""
    @State private var currentModelType = sceneModelType.alignment
    ...
    var body: some View {
        ZStack{
            MeshRendererView.shared[self.currentModelType]
            VStack{
                HStack(spacing: 30){
                    Button("←"){
                ...
                if(self.appData.rcProjectLoaded){ /*self.createBottomBar()*/ }
            }
    ...
    private func statusMessage() -> some View {
        HStack {
            ...
            } else if(!self.appData.rcNodeConnLost){
                Text("Missing connection - lost connection to RC Node").font(.caption).foregroundColor(Color.red);
            }
            else if(!self.appData.rcProjectLoaded){
                Text("Missing project - create or open a new project by clicking on project name.").font(.caption).foregroundColor(Color.red);
            } else if(self.appData.rcProjectChanging) {
                HStack(spacing: 10){
                    ProgressView().progressViewStyle(CircularProgressViewStyle(tint: .white)).scaledToFit().padding([.horizontal],10)
                    
                    /*if(self.appData.mediaUploadState != nil){
                        Text("Uploading files to CR").foregroundColor(.white).font(.caption)
                    } else if(!self.appData.rcProjectLoaded){
            ...
            } else if(!CRFly.shared.savedAlbumController.mediaSavable){
